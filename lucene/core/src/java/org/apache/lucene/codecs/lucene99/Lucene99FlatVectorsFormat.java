...
public final class Lucene99FlatVectorsFormat extends FlatVectorsFormat {
  ...
  // VECTROID: This constructor is used from backward_codecs which we don't package ourselves, but
  // we use Lucene's package, which doesn't see the new constructor.
  // We use `false` for `preload`, which is good if the index is using the old scalar quantization
  // codec (the Lucene99ScalarQuantizedVectorsFormat), which is the only one which is currently
  // using this constructor.
  public Lucene99FlatVectorsFormat(FlatVectorsScorer vectorsScorer) {
    this(vectorsScorer, false, true);
  }

  /** Constructs a format */
  public Lucene99FlatVectorsFormat(FlatVectorsScorer vectorsScorer, boolean preload, boolean useTemporaryVectorFile) {
    super(NAME);
  ...
  public FlatVectorsWriter fieldsWriter(SegmentWriteState state) throws IOException {
    return new Lucene99FlatVectorsWriter(state, vectorsScorer, useTemporaryVectorFile);
  }

  @Override
  public FlatVectorsReader fieldsReader(SegmentReadState state) throws IOException {
    return new Lucene99FlatVectorsReader(state, vectorsScorer, preload);
  }
