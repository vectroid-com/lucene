...
public class Lucene99HnswVectorsFormat extends KnnVectorsFormat {
  ...
  public Lucene99HnswVectorsFormat(int maxConn, int beamWidth) {
    this(maxConn, beamWidth, DEFAULT_NUM_MERGE_WORKER, null, HNSW_GRAPH_THRESHOLD, VERSION_CURRENT, true);
  }
  ...
  public Lucene99HnswVectorsFormat(int maxConn, int beamWidth, int tinySegmentsThreshold) {
    this(
        maxConn, beamWidth, DEFAULT_NUM_MERGE_WORKER, null, tinySegmentsThreshold, VERSION_CURRENT, true);
  }
   ...
   * @param maxConn the maximum number of connections to a node in the HNSW graph
   * @param beamWidth the size of the queue maintained during graph construction.
   * @param numHnswThreads number of threads that will be used when building an HNSW graph. If larger
   *     than 1, a non-null {@link ExecutorService} must be passed as mergeExec
   ...
   */
  public Lucene99HnswVectorsFormat(
      int maxConn, int beamWidth, int numHnswThreads, ExecutorService mergeExec, boolean useTemporaryVectorFile) {
    this(maxConn, beamWidth, numHnswThreads, mergeExec, HNSW_GRAPH_THRESHOLD, VERSION_CURRENT, useTemporaryVectorFile);
  }
   ...
   * @param maxConn the maximum number of connections to a node in the HNSW graph
   * @param beamWidth the size of the queue maintained during graph construction.
   * @param numHnswThreads number of workers (threads) that will be used when doing merge. If
  ...
  public Lucene99HnswVectorsFormat(
    ...
    this(maxConn, beamWidth, numHnswThreads, mergeExec, tinySegmentsThreshold, VERSION_CURRENT, useTemporaryVectorFile);
  }
  ...
  Lucene99HnswVectorsFormat(
    ...
    flatVectorsFormat = new Lucene99FlatVectorsFormat(FlatVectorScorerUtil.getLucene99FlatVectorsScorer(), true, useTemporaryVectorFile);
    ...
    if (numHnswThreads == 1 && mergeExec != null) {
      throw new IllegalArgumentException(
