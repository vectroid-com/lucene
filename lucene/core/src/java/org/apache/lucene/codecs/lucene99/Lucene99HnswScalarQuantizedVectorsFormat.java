...
public class Lucene99HnswScalarQuantizedVectorsFormat extends KnnVectorsFormat {
  ...
  public Lucene99HnswScalarQuantizedVectorsFormat() {
    this(DEFAULT_MAX_CONN, DEFAULT_BEAM_WIDTH, DEFAULT_NUM_MERGE_WORKER, 7, false, null, null, true);
  }
  ...
  public Lucene99HnswScalarQuantizedVectorsFormat(int maxConn, int beamWidth) {
    this(maxConn, beamWidth, DEFAULT_NUM_MERGE_WORKER, 7, false, null, null, true);
  }
   ...
   * @param maxConn the maximum number of connections to a node in the HNSW graph
   * @param beamWidth the size of the queue maintained during graph construction.
   * @param numHnswThreads number of workers (threads) that will be used when building the HNSW
   *     graph, both during initial indexing, and during a merge. If larger than 1, a non-null
   *     {@link ExecutorService} must be passed as mergeExec
  ...
  public Lucene99HnswScalarQuantizedVectorsFormat(
    ...
    this.flatVectorsFormat =
        new Lucene99ScalarQuantizedVectorsFormat(confidenceInterval, bits, compress, useTemporaryVectorFile);
  }
